...
const maxInArray = (arr) => {
  ...
  return arr.reduce((prev, curr) => {
    // math.max method used to compare the previous value and current and returns the greatest.
    // This process continues until the entire array is processed, resulting in the maximum value being returned by the "reduce" method.
...
const getCourseKeys = (obj) => {
  // Inside the function, the "Object.keys" method is used to extract all the keys from the object passed as an argument.
...
const checkValues = (obj, value) => {
  // The "includes" method is then used on this array to check whether the value passed as the second argument exists in the array of values.
...
const updateNumbers = (obj) => {
 // seperate values passed in argument and put them in an array
 let values = Object.values(obj);
 let keys = Object.keys(obj);

 let combinedArr = [];
// A nested for loop is then used to iterate over each key and each value, and push the string "key: value" to the "combinedArr" array.
 for (let key of keys) {
  for (let value of values) {
    combinedArr.push(`${key}: ${value}`)
  }
}
// Finally, the function returns the "combinedArr" array, which contains the updated data in the desired format.
return combinedArr;
};
...
const getHouses = (arr) => {
  let houses = [];
// The "map" method is used on the input array "arr" to iterate over each object in the array and extract the "house" property from it. The extracted "house" value is then pushed into the "houses" array.
